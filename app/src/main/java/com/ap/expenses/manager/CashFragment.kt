...
import com.ap.expenses.manager.databinding.FragmentCashBinding
...
class CashFragment : Fragment(), CashTransactionAdapter.OnTransactionClickListener {

    private var _binding: FragmentCashBinding? = null
    private val binding get() = _binding!!

    private lateinit var db: AppDatabase
    private lateinit var transactionAdapter: CashTransactionAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.fabAddCash.setOnClickListener { showAddTransactionDialog() }
        binding.btnDatePickerCash.setOnClickListener { showDatePickerDialog() }
        binding.swipeRefreshLayoutCash.setOnRefreshListener {
            binding.swipeRefreshLayoutCash.isRefreshing = true
            loadTransactions()
        }
        binding.tvFilterStatusCash.setOnClickListener { showFilterDialog() }
    }

    private fun setupRecyclerView() {
        transactionAdapter = CashTransactionAdapter(emptyList(), this)
        binding.recyclerViewCash.apply {
            ...
            db.cashTransactionDao().getFilteredTransactions(
                ...
                val groupedTransactions = if (isDateRangeFilter) {
                    sortedTransactions.groupBy { it.date }.flatMap { (date, txns) ->
                        listOf(CashTransaction(date = date, type = "HEADER", amount = 0.0, person = "", description = null, time = null, id = -1)) + txns
                    }
                } else {
                    sortedTransactions
                }
                ...
                binding.swipeRefreshLayoutCash.isRefreshing = false
            }
        }
    }

    private fun sortTransactions(transactions: List<CashTransaction>, sortOrder: Int): List<CashTransaction> {
    ...
    private fun updateTotalsUI(transactions: List<CashTransaction>) {
    ...
    private fun showFilterDialog() {
        ...
        MaterialAlertDialogBuilder(requireContext())
            ...
            .setPositiveButton("Apply") { _, _ ->
                ...
                binding.tvFilterStatusCash.text = "Filters Applied. Tap to change."
                updateDateButtonText()
                loadTransactions()
            }
    ...
    private fun clearFilters() {
        ...
        currentSortOrder = 0
        binding.tvFilterStatusCash.text = "No active filters. Tap to add."
        updateDateButtonText()
    }

    override fun onEditClick(transaction: CashTransaction) {
        if (transaction.type != "HEADER") {
            showAddTransactionDialog(transaction)
        }
    }

    override fun onDeleteClick(transaction: CashTransaction) {
    ...
    private fun updateDateButtonText() {
        if (isDateRangeFilter) {
            binding.btnDatePickerCash.text = "${uiDateFormat.format(apiDateFormat.parse(filterStartDate)!!)} - ${uiDateFormat.format(apiDateFormat.parse(filterEndDate)!!)}"
        } else {
            binding.btnDatePickerCash.text = uiDateFormat.format(apiDateFormat.parse(filterStartDate)!!)
        }
    }

    private fun showAddTransactionDialog(transactionToEdit: CashTransaction? = null) {
        val dialogBinding = DialogAddTransactionBinding.inflate(layoutInflater)
        dialogBinding.spinnerPaymentAppDialog.visibility = View.GONE
        val title = if (transactionToEdit == null) "Add Cash Transaction" else "Edit Cash Transaction"
            ...
            saveButton.setOnClickListener {
                ...
                lifecycleScope.launch {
                    if (transactionToEdit == null) {
                        ...
                        val updatedTransaction = transactionToEdit.copy(type = type, amount = amount, person = person, description = description, time = selectedTime)
